...
class ClaudeText:
    ...
    def health_check(self) -> HealthStatus:
        return HealthStatus(ok=False, detail="Claude text adapter not implemented yet — see master plan §11 M1")

    def generate(self, req: TextRequest) -> TextResult:
        raise NotImplementedError("call the Claude API here")


def get_adapter(model: str, options: dict) -> ClaudeText:
    return ClaudeText(model, options)
