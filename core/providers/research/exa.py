...
class ExaResearch:
    ...
    def health_check(self) -> HealthStatus:
        return HealthStatus(ok=False, detail="Exa adapter not implemented yet — see master plan §11 M1")

    def search(self, req: ResearchQuery) -> ResearchResult:
        raise NotImplementedError("call the Exa search API here")


def get_adapter(model: str, options: dict) -> ExaResearch:
    return ExaResearch(model, options)
